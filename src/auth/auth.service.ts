...
export class AuthService {
  ...
  async signIn(loginDto: LoginDto) {
    ...
    if (!pwMatched) throw new ForbiddenException('Password does not match');
  ...
  async googleLogin(googleDto: GoogleDto) {
    try {
      const x = await this.firebase.auth.verifyIdToken(googleDto.idToken);
      console.log(x);
    } catch (error) {
  ...
  async facebookLogin(facebookDto: FacebookDto) {
    try {
      const x = await this.firebase.auth.verifyIdToken(facebookDto.idToken);
      console.log(x);
    } catch (error) {
